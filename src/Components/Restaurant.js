...
function Restaurant(props) {
      ...
      .then(response => {
        ...
        setRestData({
          ...
          addressSearch:
            ...
            response.data.location.address.replace(/ /g, "+"),
  ...
  useEffect(() => {
    axios
      .get(
        `https://developers.zomato.com/api/v2.1/reviews?res_id=${restaurantID}&count=${maxCount}`,
        header
      )
      .then(response => {
      ...
      <div className="restaurant-container">
        ...
        <div className="restaurant-address">
          <div>
            ...
            {/*TODO NEED TO HAVE THIS SPLIT INTO TWO LINES*/}
          </div>
          <a target="_blank" href={restData.addressSearch}>
            <button> Google Maps</button>
          </a>{" "}
          {/*TODO this needs to be functional lol*/}
        </div>
        ...
        <div className="restaurant-reviews">
          {" "}
          {/*if the review had a negative sentiment, put here*/}
          <h2>Reviews</h2>
          {reviews.map(item => {
            if (item.result.score < 2) {
              return <Review thisReview={item} />;
            }
          })}
        </div>
